...
class RecSysAlgorithms():
    ...
    def __init__(self, graph, selected_method, selected_attribute, edges_at_distance_2):
        ...
        """
        new_algolist = []
        for algoname in self.selected_methods:
            
            if algoname not in self.available_methods:
                
                print("Not implemented method: %s"%algoname)
            
            else:
                
                new_algolist.append(algoname)
        
        
        self.selected_methods = new_algolist
